...
CollectionScanner::displayHelp()
{
    ...
    qerr << qPrintable( tr( "Amarok Collection Scanner" ) ) << endl;
    qerr << qPrintable( tr( "(C) 2003-2009 by Max Howell, Mark Kretschmann, Alexandre Oliveira, Dan Meltzer, Jeff Mitchell, and the other Amarok Developers" ) ) << endl << endl;
    qerr << qPrintable( tr( "Note: For debugging purposes this application can be invoked from the command line,\nbut it will not actually build a collection this way without the Amarok player." ) ) << endl << endl;
    qerr << qPrintable( tr( "IRC:\nserver: irc.freenode.net / channels: #amarok, #amarok.de, #amarok.es, #amarok.fr\n\nFeedback:\namarok@kde.org" ) ) << endl << endl;
    qerr << qPrintable( tr( "Usage: amarokcollectionscanner [options] +Folder(s)" ) ) << endl << endl;
    qerr << qPrintable( tr( "User-modifiable Options:" ) ) << endl;
    qerr << qPrintable( tr( "+Folder(s)            : Folders to scan; when using -b and -i, the path to the file generated by Amarok containing the list of folders" ) ) << endl;
    qerr << qPrintable( tr( "-h, --help            : This help text" ) ) << endl;
    qerr << qPrintable( tr( "-r, --recursive       : Scan folders recursively" ) ) << endl;
    qerr << qPrintable( tr( "-i, --incremental     : Incremental scan (modified folders only).  Only use this in batch mode." ) ) << endl;
    qerr << qPrintable( tr( "-p, --importplaylists : Import playlists" ) ) << endl;
    qerr << qPrintable( tr( "-s, --restart         : Restart the scanner in its last position, after a crash" ) ) << endl;
    qerr << qPrintable( tr( "-b, --batch           : Run in batch mode" ) ) << endl;
    qerr << qPrintable( tr( "--rpath <path>        : In full-scan batch mode, specifies the path to prepend to entries (default is the current directory" ) ) << endl;
    qerr.flush();
    ::exit(0);
}
