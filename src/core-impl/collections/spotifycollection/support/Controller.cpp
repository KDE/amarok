...
#define DEBUG_PREFIX "Spotify::Controller"
...
namespace Spotify {
    ...
    Controller::resolve( const QString &artist, const QString &album, const QString &title )
    {
        ...
        debug() << "Querying spotify for artist name = " << artist
    ...
    Controller::processStatus( KJob *statusJob )
    {
        if( statusJob->error() != 0 ) {
            // debug() << "Error getting status from Spotify";
            emit spotifyError( Spotify::Controller::ErrorState( ExternalError ) );
            return;
        }
        ...
        {
            debug() << "Expected a service name from Spotify, received none";
            emit spotifyError( Spotify::Controller::ErrorState( MissingServiceName ) );
            return;
        }
        if( parsedStatus.value("name") != QString( "spotify" ) )
        {
            debug() << "Expected Spotify, got response from some other service";
            emit spotifyError( Spotify::Controller::ErrorState( WrongServiceName ) );
            return;
        }
        
        debug() << "All good! Emitting spotifyReady()";
        emit spotifyReady();
    }
    ...
    Controller::processQuery( KJob *queryJob )
    {
        ...
        if( queryJob->error() != 0 )
        {
            debug() << "Error getting qid from Spotify";
            emit spotifyError( Spotify::Controller::ErrorState( ExternalError ) );
            return;
        }
        ...
        {
            debug() << "Expected qid in Spotify's response, but didn't get it";
            emit spotifyError( Spotify::Controller::ErrorState( MissingQid ) );
            return;
        }
        
        Query* query = new Query( parsedQuery.value( "qid" ).toString(), this, m_queriesShouldWaitForSolutions );
        
        debug() << "All good! Emitting queryReady( Spotify::Query* )...";
        emit queryReady( query );
        
        connect( query, SIGNAL( spotifyError( Spotify::Controller::ErrorState ) ),
                 this, SIGNAL( spotifyError( Spotify::Controller::ErrorState ) ) );
    }
}
