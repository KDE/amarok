...
public:
    int column;
    MyCheckListItem( int c, QListView *v, const QString &s, Type t ):
        QCheckListItem( v, s, t ), column( c ) { }
    virtual void paintCell( QPainter * p, const QColorGroup &cg, int c, int w, int a )
    {
        QFont f = p->font();
        f.setBold( isOn() );
        p->setFont( f );
        QCheckListItem::paintCell( p, cg, c, w, a );
    }
    ...
    : QHBox( parent, name )
{
    ...
    for( int i = h->count() - 1; i >= 0; --i )
    {
        ...
        if(s != MetaBundle::Mood || AmarokConfig::showMoodbar())
            ( new MyCheckListItem( s, m_list, MetaBundle::prettyColumnName( s ), QCheckListItem::CheckBox ) )
