...
struct SqlQueryMaker::Private
{
    ...
    bool blocking;
    bool used;
    // lists for storing acoustic fingerprint similarity filter values
    QList<Meta::TrackPtr> similarityTracks;
    QList<Fingerprint::Similarity> similarityValues;
    QList<NumberComparison> similarityComparisons;
};
...
SqlQueryMaker::reset()
{
    ...
    d->used = false;
    d->data.clear();
    d->similarityTracks.clear();
    d->similarityValues.clear();
    d->similarityComparisons.clear();
    return this;
}
...
SqlQueryMaker::buildQuery()
{
    ...
    if ( d->maxResultSize > -1 )
        /* If we have similarity filters (which cannot be put into a SQL query), we
           need to grab more rows from the database in case some of them get
           filtered out.  handleTracks will make sure the results are properly
           limited to maxTracks. */
        if ( ( d->similarityTracks.size() > 0 ) && d->queryType == QueryMaker::Track ) {
            // this increase factor is really just a wild-assed guess -- sth
            query += QString( " LIMIT %1 OFFSET 0 " ).arg( d->maxResultSize * ( 2 + d->similarityTracks.size() ) );
        } else {
...
SqlQueryMaker::handleTracks( const QStringList &result )
{
    ...
    // there are 31 columns listed in SqlTrack::getTrackReturnValues()
    ...
    for ( int i = 0; i < resultRows; i++ ) {
        ...
        t = reg->getTrack( row );
        // TODO: more logical error handling if the cast below fails
        KSharedPtr<SqlTrack> sqlT = KSharedPtr<SqlTrack>::staticCast( t );
        if (( d->similarityTracks.size() > 0 ) && sqlT ) {
            // drop tracks from result list if they don't match similarity criteria
            for ( int j = 0; j < d->similarityTracks.size(); j++ ) {
                if ( d->similarityComparisons[j] == QueryMaker::Equals ) {
                    if ( sqlT->calcSimilarityTo( d->similarityTracks[j] ) == d->similarityValues[j] )
                        tracks.append( t );
                } else if ( d->similarityComparisons[j] == QueryMaker::GreaterThan ) {
                    if ( sqlT->calcSimilarityTo( d->similarityTracks[j] ) > d->similarityValues[j] )
                        tracks.append( t );
                } else if ( d->similarityComparisons[j] == QueryMaker::LessThan ) {
                    if ( sqlT->calcSimilarityTo( d->similarityTracks[j] ) < d->similarityValues[j] )
                        tracks.append( t );
                } else {
                    kDebug() << "SqlQueryMaker doesn't know how to deal with comparison type" << d->similarityValues[j];
                }
            }
        } else {
            // there are no similarity criteria, so add all tracks to the result list
            // also falls back here if the cast failed
            tracks.append( t );
        }
        if ( ( d->maxResultSize > 0 ) && ( tracks.size() >= d->maxResultSize ) )
            break;
    }
...
QueryMaker*
SqlQueryMaker::addSimilarityFilter( const Meta::TrackPtr &track, const double value, const NumberComparison compare )
{
    d->similarityTracks.append( track );
    d->similarityValues.append( static_cast<Fingerprint::Similarity>(value) );
    d->similarityComparisons.append( compare );
