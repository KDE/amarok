...
QMap<Q3ListViewItem*, CoverFetcher*> *CollectionDB::itemCoverMap = new QMap<Q3ListViewItem*, CoverFetcher*>();
...
CollectionDB::createDragPixmap( const KUrl::List &urls, QString textOverRide )
{
    ...
    {
        text = i18np( "One item", "%1 items", songs + remoteUrls + playlists );
    }
    else if( songs > 0 )
    {
        if( correctAlbumCount ) {
            text = i18nc( "X songs from X albums", "%2 from %1" );
            text = text.arg( albums == 1 && !album.isEmpty() ? album : i18np( "one album", "%1 albums",albums ) );
        }
        else
            text = "%1";
        text = text.arg( songs == 1 && !song.isEmpty() ? song : i18np( "One song", "%1 songs", songs ) );
    }
    else if( playlists > 0 )
        text = i18np( "One playlist", "%1 playlists", playlists );
    else if ( remoteUrls > 0 )
        text = i18np( "One remote file", "%1 remote files", remoteUrls );
    else
...
CollectionDB::makeShadowedImage( const QString& albumImage, bool cache )
{
    ...
    if ( QFile::exists( folder + file ) )
        ...
        shadow = shadow.scaled( original.width() + shadowSize, original.height() + shadowSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation );
...
CollectionDB::findAmazonImage( const QString &artist, const QString &album, uint width )
{
    ...
    if ( imageDir.exists( key ) )
    {
        if ( width > 1 )
        {
            ...
            img.scaled( width, width, Qt::KeepAspectRatio, Qt::SmoothTransformation ).save( cacheCoverDir().filePath( widthKey + key ), "PNG" );
...
CollectionDB::findDirectoryImage( const QString& artist, const QString& album, uint width )
{
    ...
    if ( !values.isEmpty() )
    {
        ...
        if ( width > 1 )
        {
            ...
            {
                QImage img( image );
                img.scaled( width, width, Qt::KeepAspectRatio, Qt::SmoothTransformation ).save( path, "PNG" );
            }
            return path;
        }
        else //large image
...
CollectionDB::notAvailCover( const bool withShadow, int width )
{
    ...
    {
        m_noCover.scaled( width, width, Qt::KeepAspectRatio, Qt::SmoothTransformation ).save( cacheCoverDir().filePath( widthKey + "nocover.png" ), "PNG" );
        s = cacheCoverDir().filePath( widthKey + "nocover.png" );
    }
...
CollectionDB::initialize()
{
...
#endif
    {
        m_dbConfig = new SqliteConfig(
                        Amarok::config( "Sqlite" ).readEntry( "location",
          ...
          || Amarok::config( "Collection Browser" ).readEntry( "Database Stats Version", 0 ) != DATABASE_STATS_VERSION )
    {
        ...
        else        //Incrementally update the stats table to reach the present version
        {
            ...
            if( prev < 9 )
            {
                ...
                MountPointManager::instance()->setCollectionFolders( Amarok::config( "Collection" ).readEntry( "Collection Folders", QStringList() ) );
