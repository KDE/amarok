...
public:
    ...
    typedef QList<EmbeddedImage> EmbeddedImageList;
    ...
    bool matchesSimpleExpression( const QString &expression, const QList<int> &columns ) const;
    ...
    bool matchesExpression( const QString &expression, const QList<int> &defaultColumns ) const;

    /** Returns whether the bundle matches the pre-parsed expression \p parsedData.
        The tags in \p defaultColumns are checked for matches where the expression doesn't specify any manually.
        @see ExpressionParser */
    bool matchesParsedExpression( const ParsedExpression &parsedData, const QList<int> &defaultColumns ) const;
...
protected:
    ...
    virtual void aboutToChange( const QList<int> &columns );

    /** Convenience method. */
    void aboutToChange( int column );

    /** Called after the tags in \p columns are changed. */
    virtual void reactToChanges( const QList<int> &columns );
...
inline void MetaBundle::aboutToChange( const QList<int>& ) { }
inline void MetaBundle::aboutToChange( int column ) { aboutToChange( QList<int>() << column ); }
inline void MetaBundle::reactToChange( int column ) { reactToChanges( QList<int>() << column ); }
