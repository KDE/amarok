...
{
    qDebug() << qPrintable( tr( "Amarok Collection Scanner\n\nNote: For debugging purposes this application can be invoked from the command line, but it will not actually build a collection this way." ) );
    qDebug() << qPrintable( tr( "(C) 2003-2009, The Amarok Developers" ) );
    qDebug() << qPrintable( tr( "IRC:\nserver: irc.freenode.net / channels: #amarok, #amarok.de, #amarok.es, #amarok.fr\n\nFeedback:\namarok@kde.org" ) );
    qDebug() << qPrintable( tr( "Usage: amarokcollectionscanner [options] +Folder(s)" ) );
    qDebug() << qPrintable( tr( "\n\nUser-modifiable Options:" ) );
    qDebug() << qPrintable( tr( "+Folder(s)            : Folders to scan; when using -b and -i, the path to the file generated by Amarok containing the list of folders" ) );
    qDebug() << qPrintable( tr( "-h, --help            : This help text" ) );
    qDebug() << qPrintable( tr( "-r, --recursive       : Scan folders recursively" ) );
    qDebug() << qPrintable( tr( "-i, --incremental     : Incremental scan (modified folders only).  Only use this in batch mode." ) );
    qDebug() << qPrintable( tr( "-p, --importplaylists : Import playlists" ) );
    qDebug() << qPrintable( tr( "-s, --restart         : Restart the scanner in its last position, after a crash" ) );
    qDebug() << qPrintable( tr( "-b, --batch           : Run in batch mode" ) );
    qDebug() << qPrintable( tr( "--rpath <path>        : In full-scan batch mode, specifies the path to prepend to entries (default is the current directory" ) );
    ::exit(0);
}
